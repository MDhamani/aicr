		...
		},
		{
			...
			mutate: func(f *fakeFetcher) {
				f.gets["apps/v1/DaemonSet/kube-system/nccl-rdma-installer"] = map[string]any{
					"apiVersion": "apps/v1",
					"kind":       "DaemonSet",
					"metadata":   map[string]any{"name": "nccl-rdma-installer", "namespace": "kube-system"},
					"status":     map[string]any{"desiredNumberScheduled": 2, "numberReady": 1},
				}
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			fetcher.addGet("apps/v1", "DaemonSet", "kube-system", "nccl-rdma-installer", map[string]any{
				...
				"kind":       "DaemonSet",
				"metadata":   map[string]any{"name": "nccl-rdma-installer", "namespace": "kube-system"},
				"status":     map[string]any{"desiredNumberScheduled": 2, "numberReady": 2},
			})
