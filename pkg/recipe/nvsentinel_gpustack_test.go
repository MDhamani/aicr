...
func TestNVSentinelConfigurationMatrix(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "OKE oci-default: image driver + OKE's plugin, no driver pod",
			criteria: &Criteria{
			...
			profile:    "gpuStack=oci-default",
			wantAssume: ptr(true),
		},
		{
			name: "OKE operator-plugin: image driver, operator plugin, still no driver pod",
			criteria: &Criteria{
				Service:     CriteriaServiceOKE,
				Accelerator: CriteriaAcceleratorA100,
				OS:          CriteriaOSOracleLinux,
			...
			profile:    "gpuStack=operator-plugin",
			wantAssume: ptr(true),
		},
		{
...
func TestOKENVSentinelValueIsProfileOwned(t *testing.T) {
	...
	if assume, ok := nestedBool(values, "labeler", "assumeDriverInstalled"); !ok || !assume {
		t.Fatalf("OKE labeler.assumeDriverInstalled = %v (set: %v), want true under oci-default", assume, ok)
	}

	// ... and profile-owned.
	selected := result.Metadata.SelectedProfile
	if selected == nil || selected.Name != "gpuStack" || selected.Value != "oci-default" {
		t.Fatalf("selectedProfile = %#v, want gpuStack=oci-default", selected)
	}
