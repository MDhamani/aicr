...
| **gcp-driver-installer** | Google's cos-gpu-installer DaemonSet as an AICR-managed, values-gated component. Present in every GKE COS recipe; renders only under the `gpuStack=operator-selfdriver` profile value, where it installs the pinned NVIDIA driver on pools created with `gpu-driver-version=disabled` and stamps the NFD ownership marker the value's readiness constraint asserts. GKE-specific. | — |
...
See GKE's [GPU node-pool guide](https://cloud.google.com/kubernetes-engine/docs/how-to/gpus) for the authoritative pool-creation and driver-installer procedures. The [NVIDIA GPU Operator GKE guide](https://docs.nvidia.com/datacenter/cloud-native/gpu-operator/latest/google-gke.html) documents the same `gpu-driver-version=disabled` + driver-installer DaemonSet combination the `driver-installer` value builds on. The third value, `operator-selfdriver` ([#1716](https://github.com/NVIDIA/aicr/issues/1716)), keeps the same pool shape but the bundle's `gcp-driver-installer` component carries that DaemonSet: the driver version is pinned in the recipe, upgrades roll with the bundle, and the installer stamps `feature.node.kubernetes.io/gcp-driver-installer=true` (via an NFD local feature file) — the durable marker its readiness constraint asserts and `driver-installer`'s symmetrically denies (ADR-015 DD5).
